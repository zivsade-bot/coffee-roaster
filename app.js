...
// Setup full-area touch control for time picker
function setupFullAreaTouch() {
    const wheelsContainer = document.querySelector('.time-picker-wheels');
    const minutesWheel = document.getElementById('minutesWheel');
    const secondsWheel = document.getElementById('secondsWheel');
    
    if (!wheelsContainer || !minutesWheel || !secondsWheel) return;
    ...
    const newContainer = wheelsContainer.cloneNode(true);
    wheelsContainer.parentNode.replaceChild(newContainer, wheelsContainer);
    
    // Get fresh references after cloning
    const container = document.querySelector('.time-picker-wheels');
    const minutesWheelFresh = document.getElementById('minutesWheel');
    const secondsWheelFresh = document.getElementById('secondsWheel');
    ...
    // Helper function to determine active wheel
    function determineActiveWheel(clientX) {
        const containerRect = container.getBoundingClientRect();
        const relativeX = clientX - containerRect.left;
        const containerWidth = containerRect.width;
        
        // Left half = minutes, right half = seconds
        return relativeX < containerWidth / 2 ? minutesWheelFresh : secondsWheelFresh;
    }
    
    // Start event (touch or mouse)
    function handleStart(e) {
        ...
        activeWheel = determineActiveWheel(coords.x);
        
        if (activeWheel) {
            initialScrollTop = activeWheel.scrollTop;
        }
    }
    
    // Move event (touch or mouse)
    function handleMove(e) {
        if (!activeWheel || !isDragging) return;
        
        const coords = getCoordinates(e);
        const deltaY = startY - coords.y;
        
        // Scroll the active wheel
        activeWheel.scrollTop = initialScrollTop + deltaY;
    }
    
    // End event (touch or mouse)
    function handleEnd() {
        isDragging = false;
        activeWheel = null;
    }
